...
})({"lCMR6":[function(require,module,exports) {
...
var _listworkexperience = require("./listworkexperience");
//DOM event
...
//variabler globala
const worklists = new (0, _listworkexperience.WorkExperienceList)(token || "");
...
async function submitform(event) {
    ...
    if (event.target instanceof HTMLFormElement) {
        ...
        if (addok == 201) addrow(data);
        else alert("Kontrollera inmatningsdata");
    }
}
...
async function submitadduser(event) {
    ...
    try {
        ...
        console.error("N\xe5tgick fel" + error);
    }
}
async function submitloggain(event) {
    //hindrar default inställningar på form
    event.preventDefault();
    try {
...
async function init() {
    try {
        const token = localStorage.getItem("jwt");
        if (token) worklists.settoken(token);
        const experiences = await worklists.getalldata();
        experiences.forEach((element)=>{
            addrow(element);
        });
        if (form) form.addEventListener("submit", submitform);
        if (loggain) loggain.addEventListener("submit", submitloggain);
        if (adduser) adduser.addEventListener("submit", submitadduser);
    } catch (error) {
...
function addrow(data) {
    ...
    if (experiencelist) {
        ...
        summary.textContent = "F\xf6retag: " + data.companyname + " - " + data.jobtitle + " - " + getYear(data.startdate) + "-" + getMonth(data.startdate) + " - " + getYear(data.enddate) + "-" + getMonth(data.enddate);
    ...
    } else console.error("n\xe5t gick fel vid h\xe4mtning av tabell.");
}
...
},{"./listworkexperience":"391xQ"}],"391xQ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "WorkExperienceList", ()=>WorkExperienceList);
...
class WorkExperienceList {
    ...
    async Initdatabasedata() {
        try {
            ...
            this.WorkExperiencearray = temparray.map((element)=>new (0, _workexperience.workexperience)(element._id, element.companyName, element.jobTitle, element.location, new Date(element.startDate), new Date(element.endDate), element.description));
        } catch (err) {
            console.error("n\xe5t gick fel vid h\xe4mtning av data:", err);
        }
    }
    async deleteworkexperience(id) {
        try {
            ...
            console.error("n\xe5t gick fel vid borttagning av data:", err);
        }
    }
    async addworkexperience(data) {
        try {
            ...
            if (!companyname || !jobtitle || !location || !startdate || !enddate || !description) console.error("Indata saknas.");
            else if (isNaN(new Date(startdate).getTime()) || isNaN(new Date(enddate).getTime())) console.error("Datum inkorrekt");
            else {
            ...
            console.error("n\xe5t gick fel vid till\xe4gg av data:", err);
            return null;
        }
    }
...
},{"./workexperience":"2aDfm","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"2aDfm":[function(require,module,exports) {
//klass för att hantera workexperience data
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "workexperience", ()=>workexperience);
class workexperience {
    constructor(id, companyname, jobtitle, location, startdate, enddate, description){
        this.id = id;
...
},{}]},["lCMR6","3rz9v"], "3rz9v", "parcelRequireac4b")
